...
def is_edge_point (x, y, z, mask, mask_resolution, rmin, rmax):
    
    is_edge = False
    #-------------------------------------------------------------------
    # Is the point within 10 Mpc/h of the survey boundary?
    #-------------------------------------------------------------------
    ...
    for i in range(pts.shape[1]):
        ...
        if not not_in_mask:
            is_edge = is_edge_point(curr_pt[0], curr_pt[1], curr_pt[2],
                                    mask, mask_resolution, rmin, rmax)
...
def prep_V2_cat(V2_galzones, V2_zonevoids, data_table_vl):
    ...
    # Sort the catalogs
    data_table_vl.sort('gal') #this column will only exists if the galaxies passed in as a VoidCatalog.galaxies object field or if the user otherwise configures a 'gal' column for the galaxies
    V2_galzones.sort('gal')
    
    # Ensure that each galaxy in the catalog has a zone in the galzones file
    ...
    void = V2_zonevoids[V2_galzones['zone']]['void1']
    void[V2_galzones['zone']==-1] = -1
    data_table_vl['in_void'] = (void > -1)
    #data_table_vl = data_table_vl[V2_galzones['zone'] != -1]
    
    return data_table_vl
...
class OverlapCalculator():
    ...
    def __init__ (self, data_table_V1, data_table_V2, title1, title2, mask_file, rmin, rmax, galaxy_table_V1 = None, galaxy_table_V2 = None, zone_table_V1 = None, zone_table_V2 = None, use_mask = True, V1_algorithm="VF", V2_algorithm="VF",mask_tuple = None, num_cpus=1):
        """Formates a V2 catalog for use with the void overlap calclator
        ...
        rmax : float
            The comoving maximum distance within the survey volume
        galaxy_table_V1: Astropy Table or None
            The galaxy catalog used to generate the first or both V2 catalogs     
        galaxy_table: Astropy Table or None
            The galaxy catalog used to generate the second V2 catalog    
        ...
        if V1_algorithm=="V2":
            data_table_V1 = prep_V2_cat(data_table_V1, zone_table_V1, galaxy_table_V1)
        ...
        if V2_algorithm=="V2":
            if galaxy_table_V2 is None:
                galaxy_table_V2 = galaxy_table_V1
            data_table_V2 = prep_V2_cat(data_table_V2, zone_table_V2, galaxy_table_V2)
        else:
...
class SingleCalculator():
    """Compares the voulume overlap of one void catalog. 
    
    WARNING: This class assumes that V2 catalogs are generated 
    from the same underlying galaxy catalog. The code must be manually edtied if this isn't the case.
    """
    
    def __init__ (self, data_table_V1, title1, mask_file, rmin, rmax, galaxy_table_V1 = None, zone_table_V1 = None, use_mask = True, V1_algorithm="VF",mask_tuple = None):
        """Formates a V2 catalog for use with the void overlap calclator
